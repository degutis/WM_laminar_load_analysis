...
[clusters_test1, p_values_test1, ~, ~] = permutest(test1, matrixShuffled1, 0, 0.05, 10000, 0);
cluster_above_chance_test1 = zeros([matrixSize(1),matrixSize(2)]);
for c=1:length(clusters_test1)
    if p_values_test1(c)<0.05
...
[clusters_test2, p_values_test2, ~, ~] = permutest(test2, matrixShuffled2, 0, 0.05, 10000, 0);
cluster_above_chance_test2 = zeros([matrixSize(1),matrixSize(2)]);
for c=1:length(clusters_test2)
    if p_values_test2(c)<0.05
