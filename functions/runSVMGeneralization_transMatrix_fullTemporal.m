function [accuracyAverage] = runSVMGeneralization_transMatrix_fullTemporal(dataSet, labels,meanTR,featureSelect,featureZscore,shuffleLabels)
...
GN_min_train = [GN_min_train;GN_min_train;GN_min_train;GN_min_train];
...
trainValIndex = zeros(size(dataSet,2),length(GN_min_train));
...
parfor trainIndex = 1:numIterations
     for testIndex=1:numIterations
        ...
        accuracyAverage(trainIndex,testIndex) = mean(cellfun(@(x) x(1), accuracy));
     end
end
